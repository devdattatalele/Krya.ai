...
def run_script_async(script_path: Optional[str] = None) -> Tuple[subprocess.Popen, str]:
    ...
    try:
        ...
        # Add a unique execution ID if not already set
        if "KRYA_EXECUTION_ID" not in env:
            env["KRYA_EXECUTION_ID"] = f"exec_{datetime.now().strftime('%Y%m%d%H%M%S')}"
...
# Get the execution ID from environment
...
if os.path.exists(flag_file):
    ...
    if time.time() - os.path.getmtime(flag_file) > 120:
        ...
        print(f"Another execution is already in progress (ID: {{EXECUTION_ID}}). Exiting.")
        sys.exit(0)

# Create the flag file with the execution ID
with open(flag_file, 'w') as ff:
    ff.write(EXECUTION_ID or str(time.time()))
...
try:
    # Execute the actual script with the same environment variables
    env = os.environ.copy()
...
"""
            ...
            # Execute the wrapper script instead with the environment variables
...
def run_script_async_with_env(script_path: Optional[str] = None, env: Optional[Dict[str, str]] = None) -> Tuple[subprocess.Popen, str]:
    """
    Execute a Python script asynchronously with custom environment variables
    ...
    try:
        ...
        if env is None:
            env = os.environ.copy()
            # Add a default execution ID if not provided
            if "KRYA_EXECUTION_ID" not in env:
                env["KRYA_EXECUTION_ID"] = f"exec_{datetime.now().strftime('%Y%m%d%H%M%S')}"
...
# Get the execution ID from environment
EXECUTION_ID = os.environ.get('KRYA_EXECUTION_ID', '')
print(f"Running script with execution ID: {{EXECUTION_ID}}")

# Create a flag file to prevent multiple executions
flag_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".execution_in_progress")
script_flag_file = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 
                               "generated_output", ".script_executed")
...
if os.path.exists(flag_file):
    ...
    if time.time() - os.path.getmtime(flag_file) > 120:
        ...
        print(f"Another execution is already in progress. Exiting.")
        sys.exit(0)

# Check if this script has already been executed with this ID
if os.path.exists(script_flag_file) and EXECUTION_ID:
    try:
        with open(script_flag_file, 'r') as f:
            stored_id = f.read().strip()
            if stored_id == EXECUTION_ID:
                print(f"This script (ID: {{EXECUTION_ID}}) has already been executed. Preventing loop.")
                sys.exit(0)
    except Exception as e:
        print(f"Error checking script execution flag: {{e}}")
...
if EXECUTION_ID:
    with open(script_flag_file, 'w') as ff:
        ff.write(EXECUTION_ID)

def cleanup_flags():
    for f in [flag_file, script_flag_file]:
        if os.path.exists(f):
            try:
                os.remove(f)
                print(f"Cleaned up flag file: {{f}}")
            except Exception as e:
                print(f"Error removing flag file {{f}}: {{e}}")
...
"""
            ...
            # Execute the wrapper script with the environment variables
            logger.info(f"Starting script with execution ID: {env.get('KRYA_EXECUTION_ID', 'none')}")
            process = subprocess.Popen(
                ["python3", wrapper_path],
                stdout=f,
                stderr=subprocess.STDOUT,
        ...
        logger.error(f"Error in run_script_async_with_env: {e}")
        raise
